...
public class MediaDao {
    ...
    public List<Media> filterMediasgenre(String genre) {

        List<Media> medias = new ArrayList<>();

        Connection connection = Database.get().getConnection();
        try {
            PreparedStatement st = connection.prepareStatement("SELECT * FROM media WHERE genre=? ORDER BY release_date DESC");
    ...
    public List<Media> filterMediastype(String genre) {
    ...
    public List<Media> filterMediasDate(String genre) {

        List<Media> medias = new ArrayList<>();

        Connection connection = Database.get().getConnection();
        try {
            PreparedStatement st = connection.prepareStatement("SELECT * FROM media WHERE release_date=? ORDER BY release_date DESC");
            st.setString(1, genre);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                medias.add(mapToMedia(rs));
